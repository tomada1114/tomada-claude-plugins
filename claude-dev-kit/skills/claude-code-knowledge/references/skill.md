...
### 「必要な時だけ取り出すバックヤードのマニュアル棚」

Skills の仕組みを理解するには、コンビニのバックヤードに置いてあるマニュアル棚をイメージしてみてください。

棚には数十冊のマニュアルが並んでいますが、普段は背表紙（タイトルと簡単な説明）だけが見えている状態です。「クリスマスケーキの予約受付が始まった」と判断した瞬間、初めてそのマニュアルを手に取り、該当するページを開きます。すべてのマニュアルを同時に開いて置いておく必要はありません。
...
| 特徴 | Skills | SubAgents |
|------|--------|-----------|
| **イメージ** | バックヤードのマニュアル棚 | 本部のサポートセンター |
...
**Skills は「バックヤードのマニュアル棚」**:
- 必要な時だけ棚から取り出して参照
- 実際に作業するのは自分自身
- 「やり方」を教えてくれる

**SubAgents は「本部のサポートセンター」**:
- 「この返品対応どうすればいい？」と電話すれば調べて回答してくれる
- 別の場所で処理されるため、自分のレジ業務は中断せずに済む
