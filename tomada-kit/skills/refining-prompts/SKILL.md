...
5. **渡し先は現行世代のモデル** — 旧世代向けの定型句（再検証の強制、重要度フィルタ、思考過程の要求、
   「よく考えて」、`CRITICAL: MUST` 系の強調）は現在は逆効果。→ [references/prompt-patterns.md](references/prompt-patterns.md)
...
調査と推測で埋まらない本質的な分岐（目的そのもの、スコープ境界、成果物の形）だけ、選択肢と推奨を
添えて聞く。相談系は数ラウンドの対話も可。
...
プロンプト全体を **1 個の markdown コードブロック** で出力する（そのままコピペでき、新セッションが
追加の前提なしで動き出せる形）。伝えたい留意点はブロックの外ではなくプロンプト本文に入れる。
