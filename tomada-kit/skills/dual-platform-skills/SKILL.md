...
**このスキルを使わない場面**: 新規スキルのゼロ作成・監査・トラブルシュートは `creating-agent-skills` を使う。本スキルは「既存スキルの両対応化」専用。
...
- **入力**: 変換対象スキル名（`~/.claude/skills/<name>` に展開）または絶対パス。`--scope user|repo`（省略時は実体位置から自動判定）。 <!-- neutrality-ignore: N2 -->
...
1. 対象を解決し **origin** を判定（`.claude/skills/` = Claude 由来／`~/.codex/skills/`・`.agents/skills/` = Codex 由来）。 <!-- neutrality-ignore: N2 -->
...
2. 両対応検証:
   ```bash
   python3 ../creating-agent-skills/scripts/validate_skill.py <TARGET>   # Claude 妥当性（SKILL.md）
...
| script | 役割 |
|---|---|
| `scripts/classify_skill.py <dir> [--json]` | 構文検出（SKILL.md+references+templates）＋Tier(A/B/C)＋依存サブエージェント＋cross-skill＋file:line 詳細 |
| `scripts/neutrality_lint.py <dir> [--json]` | 本文への生ツール名/プラットフォームパス/独自状態パスの混入検出（N1-N4） |
