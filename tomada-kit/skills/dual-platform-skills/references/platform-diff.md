...
|---|---|---|---|
| `Agent`（旧名 `Task`。サブエージェント並列） | あり | サブエージェント機構あり（TOML 定義・別物） | 既定: Codex 版では**逐次インライン**化。注記必須 |
...
本スキルの既定方針: 外部エージェント定義に依存させず、**サブエージェントの知識を対象スキルの `references/agents/<name>.md` に抽出**して両プラットフォーム共有にする（Claude は `Agent` で、Codex は逐次インラインで同じ reference を使う）。将来オプション: `Task`→Codex TOML サブエージェント・マッピング。
