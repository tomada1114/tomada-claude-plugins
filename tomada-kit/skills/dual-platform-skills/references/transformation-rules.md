...
- [R14 実行環境（sandbox）の制約](#r14-実行環境sandboxの制約)
- [R15 ホスト組み込み能力への依存](#r15-ホスト組み込み能力への依存)
...
- **判定軸は「製品」ではなく「そのランタイムに公開されている能力」**（L13）。「Codex だから委譲不可」と書かない——同じ製品でも、スキルを起動したセッションに spawn API が公開されている場合と、まったく公開されていない場合がある（実測あり）。本文は必ず**能力条件つき**（「委譲できる環境なら…／できなければ…」）で書き、platform-notes.md 側にも「これは能力が無いランタイムでの劣化パスであって製品の機能一覧ではない」と明記する。
- **委譲が担うものは 3 種**ある。混同しない（L11）:
  ...
  - **コンテキスト隔離**（委譲先に diff・ログ・大量の読み取りを閉じ込める）→ **逐次化で失われ、時間ではなく品質に効く**。長い run・多対象の `all` 系モードでは、読み捨てるはずだった中間出力がメインに全部積み上がる。劣化注記には所要時間だけでなく**この蓄積**を書き、対象数を区切って残りを deferred と報告する等の緩和策を本文に置く（L14）。
...
## R14 実行環境（sandbox）の制約
機能の有無とは別に、**実行環境が書き込みを止める**クラスの失敗がある。Codex desktop はファイルシステム sandbox 下で動き、次が `Operation not permitted` で落ちることがある（実測）:
- **git のロック/ref 書き込み**: `.git/index.lock`・ref lock・`.git/FETCH_HEAD`。`git fetch`/`git pull`/branch 削除/worktree 操作が失敗する一方、`git status`/`add`/`commit`/`push` は通る、という**部分的な失敗**になる。
- **ユーザーホーム下のツールキャッシュ**: パッケージマネージャ・テストランナーが `~/.cache/<tool>` に書けず、依存解決やテスト実行が初回から落ちる。

対象スキルが git 書き込み系・パッケージマネージャ・テストランナーを回すなら、**本文には分岐を書かず**、platform-notes.md に症状と復旧手順を書く（本文は環境非依存のまま保つ）:
```markdown
- Git 操作の sandbox 制限 → `Operation not permitted` が出たら、その操作だけホストの承認済み昇格経路で再実行する。ad-hoc な `rm` や手動 branch 削除で迂回しない。
- ツールキャッシュの書き込み制限 → `<TOOL>_CACHE_DIR=<書き込み可能な一時ディレクトリ>` を指定して再実行する。
```
**迂回を許さないこと**が要点。ロックファイルを消す・cleanup を飛ばすといった回避は、状態を壊したまま run を続行させる。検出は [classify_skill.py](../scripts/classify_skill.py) の `sandbox_write_op`。

## R15 ホスト組み込み能力への依存
`/code-review` のような**ホスト組み込みのコマンド/スキル**への依存は、cross-skill 参照（[R5]）と違い**bridge する実体が存在しない**。inline も相対参照も不可能。ただし「無いから諦める」で終わらせず、**能力の梯子**を書く:
1. 本文はツール名を消し、**能力条件つきの中立文**にする（「セルフレビュー手段があれば実行し、無ければ…」）。
2. **上から順に、実際に取れる rung を取る**:
   - ①組み込み能力が使えるならそれ。
   - ②組み込みが無くても**汎用能力（委譲）で再構成できるなら再構成する**。ただし成立条件は**独立したコンテキスト**であること——別文脈のレビュアーが読むから意味があり、専用プロンプトはスキル内に同梱する（[R4] の canonical 規約に従って `references/agents/<name>.md` か既存のテンプレート集へ）。
   - ③どちらも無ければ劣化モード。
3. **同一コンテキストの自己レビューを②に数えない**。自分で書いた diff を自分で読み直したものを「レビュー済み」と報告させると、保証が無いのに保証があるように見える出力になる（L12 と同型）。②が取れないなら素直に③へ落とす。
4. **名前のついた劣化モードを 1 つ定義し、成果物に残す**。「続行してよい」だけでは不十分で、スキルが実行記録・最終レポートを持つなら**どの rung で走ったかを必ず出力させる**（例 `REVIEW: <effort> | DELEGATED | UNAVAILABLE`）。能力があった run と無かった run が同じレポートになる設計は不可。
5. 何が保証されなくなるのかを platform-notes.md に 1 行で書く（例「lint/型/テスト/CI は通るが、複雑性・意図とのずれ・保守性は誰も見ていない」）。
検出は [classify_skill.py](../scripts/classify_skill.py) の `builtin_slash_command`。
...
- 並列委譲 → 委譲能力が無いランタイムでは逐次実行（所要時間増＋コンテキスト隔離の喪失。緩和: <対象数を区切る等>）
- <組み込み能力への依存> → <名前つき劣化モード>（記録先: <run record / レポート>。失われる保証: <…>）
- 実行環境の制約（sandbox） → <症状> のとき <昇格 or 環境変数の付け替え> で再実行。迂回しない
- `<MCP/tmux/...>` → <フォールバック or 未対応>
```

各劣化行は **①何が使えないか ②何に劣化するか ③失うコスト（所要時間／コンテキスト量／保証レベル）** を必ず書く。「逐次実行（所要時間増）」だけで止めない（R3・R15）。先頭には「これは能力が公開されていないランタイムでの劣化パスであって、製品の機能一覧ではない」と 1 行置く。
