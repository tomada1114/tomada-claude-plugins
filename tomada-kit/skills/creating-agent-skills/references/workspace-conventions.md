...
- [Why deterministic paths beat /tmp](#why-deterministic-paths-beat-tmp)
- [Path conventions observed in production](#path-conventions-observed-in-production)
- [Computing the slug](#computing-the-slug)
- [Declaring inputs and outputs in SKILL.md](#declaring-inputs-and-outputs-in-skillmd)
- [Snapshot and restore for destructive operations](#snapshot-and-restore-for-destructive-operations)
...
2. **Pipeline handoff**: the next skill in the chain needs to find this skill's output without being told the absolute path. See "Artifact-driven handoff" in `orchestration-patterns.md` (load via SKILL.md).
...
|---|---|---|
| Design / docs pipeline | `~/<project>/design/{ticket-id}-{slug}/` | `~/acme/design/12345-add-export-button/` |
| Verification / testing scratch | `~/Desktop/testing/{YYYYMMDD}_{slug}/` | `~/Desktop/testing/20260407_config_check/` |
| Single-file outputs | `<project-root>/<deterministic-name>.<ext>` | `~/acme/webapp/CONTRIBUTING.md` |
| Per-PR artifacts | `~/<project>/pr/{pr-number}/` | `~/acme/pr/4567/` |
...
Path:    ~/acme/design/12345-csv-export/
```
...
If your skill spawns a separate process (e.g. another Claude Code session), snapshot before spawning and restore after that process completes — even on error. Wrap in a `trap`/`finally` if you can.
