...
| "two surveyor sub-agents in parallel" (step 2) | two `Task` calls in one message, `subagent_type: general-purpose`, `model: sonnet` each | run both surveys inline in the main context, one after the other, writing the same two inventory files |
| "delegate the body writing to one `sonnet` sub-agent" (step 5) | one `Task` call, `model: sonnet` | write the bodies inline |
