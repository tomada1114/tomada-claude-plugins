...
**書く量でコストが決まる。** 出力トークンは入力トークンの数倍の単価なので、メインが Fable のときのコストを支配するのは読んだ量ではなく**書いた量**。読んで判断すること自体は Fable に残してよいが、まとまった量の書き出し(実装、ドキュメント、長いファイル生成、複数ファイル編集)は Opus / Sonnet に書かせる。数行の修正や一言の追記のように、ブリーフを書くほうが長くなる短い書き込みはそのままメインでやる。
...
| 判断を含まない列挙・整形 | Haiku |
| まとまった量の書き出し(実装、ドキュメント、長いファイル生成、複数ファイル編集) | メインが Fable なら Opus / Sonnet に書かせる(仕様確定なら Sonnet、穴が残るなら Opus) / メインが Opus なら自分でやる |
