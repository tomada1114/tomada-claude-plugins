...
`AskUserQuestion` で選択肢を提示し、大きな方向性を決める（このバッチ確認の原則はPhase 4にも適用）。
...
方向性が決まったら、`AskUserQuestion` で詳細を詰める。
...
> Codex での制約と両対応に関する補足は `references/codex-notes.md` を参照。
