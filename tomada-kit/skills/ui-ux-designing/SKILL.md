...
確認ラウンドが1つ終わるごとに、design-concept.md の「決定ログ」節へ決定・却下した案・理由を追記する。中断後に再開したセッションは、質問を組み立てる前にまずこの節を読む。理由: どこまで合意済みかが残っていないと同じ質問を投げ直すことになり、理由が残っていないと後から出た異論を再検討すべきか判断できない。
...
`references/agents/research-competitors.md` のプレースホルダを埋め、`executor` サブエージェント1体（上限1体。段の選び方は `orchestrating-models` スキル）に委譲する。理由: 調査対象アプリ・評価軸・出力形式をすべて本体側で確定させて渡すため、委譲先に残るのは定型的な情報収集で判断が含まれない。
