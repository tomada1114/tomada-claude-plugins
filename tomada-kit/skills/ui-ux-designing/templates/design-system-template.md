...
- 値はトークン名で書き、生の色・px を直接書かない。名前は `tokens.css` の語彙に揃える。
...
値の正は同じディレクトリの `tokens.css`。この節には semantic 層の一覧だけを載せる。理由: 実装セッションが参照してよいのは semantic 層だけで、primitive を直接
読ませるとテーマ切替が壊れる。
...
- 太字は 700 ではなく 600 を使う。理由: 和文書体は画数が多く、700 だと小サイズで字の内側の空き（ふところ）が潰れる
...
この表の行は `check_contrast.py`（ui-ux-designing スキル同梱）の出力をそのまま貼る。理由: 記憶で書いた比率は外れる
...
- 同じディレクトリの `tokens.css`（値は差し替え済み）をアプリのスタイル読み込み経路（例:
  `src/styles/tokens.css`）へそのままコピーする。構造とトークン名は変えない
