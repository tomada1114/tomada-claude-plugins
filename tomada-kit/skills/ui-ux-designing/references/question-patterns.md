...
UI/UXデザインの方向性を決定するための質問パターン。選択肢提示による質問データとして使用する。
...
以下の JSON（`全体トーン`）は、選択肢提示のデータ形として書かれている。通常の対話でユーザーに提示する場合は、番号付きの平文に変換して提示する:

```
アプリ全体のトーン（デザインの雰囲気）はどうしますか？
1. プロフェッショナル — 落ち着いた色調、ミニマルな装飾。ビジネスツール感
2. フレンドリー — 明るい色、イラストやアニメーション多め。Duolingo風
3. モダン・クリーン — 白基調、アクセントカラー控えめ。Apple風
```

他の質問も同じ要領（`question` → 質問文、`options[].label`+`description` → 番号付き選択肢、推奨があれば末尾に「（推奨: N）」）で平文化する。以降の JSON ブロックはこの変換元データであり、そのまま複製した平文版は用意しない。
