...
**Reads:** every canonical and effective Codex instruction source (`AGENTS.md`, `AGENTS.override.md`, and configured fallback filenames), `CLAUDE.md`, `.claude/CLAUDE.md`, `CLAUDE.local.md`, `.claude/rules/**/*.md` under the root (skipping dependency and build directories), plus the relevant Codex config values; for `init`/`audit`, the project's manifests, task runners, CI config, and README; for `hooks`, `.claude/settings.json`, `.codex/hooks.json`, `.codex/config.toml` metadata, and every script under `.claude/hooks/` and `.agents/hooks/`.

**Writes:** `AGENTS.md` and `CLAUDE.md` files inside the project (root and subdirectories); in `hooks` mode also `.agents/hooks/**`, the `hooks` key of `.claude/settings.json`, and the shared projection in `.codex/hooks.json`. Existing Codex-only hooks, top-level settings, and inline `.codex/config.toml` hooks are preserved or left untouched. The project files that reference `.claude/hooks` (tests, CI, docs) may also be updated. Never `.claude/settings.local.json`. Before any modification — a snapshot at
...
Exit 1 from `inventory.py` means findings exist, not that the script failed. Read `findings`, `suggested_mode`, the effective Codex sources and budget, and the per-file `state` values (`stub`, `stub+extras`, `legacy`, `legacy-import`, `missing`, `orphan`, `malformed`). Finding codes R001–R013 (rule files) and H001–H009 (hooks) are listed at the end of the script's `--help` and explained in [references/file-contract.md](references/file-contract.md).
...
3. `python3 scripts/share_hooks.py <root> --dry-run`, then without the flag — it snapshots, relocates the scripts, rewrites the command strings to the git-toplevel form, and merges the shared projection into `.codex/hooks.json`. Existing Codex-only commands and top-level keys stay; inline `[hooks]` in `.codex/config.toml` is reported and left untouched. Claude-only events stay in `.claude/settings.json` and are reported, not an error.
...
6. Verify before reporting: every fixture in [assets/hooks/fixtures/](assets/hooks/fixtures/) through every adapted script (0 for ordinary payloads, 2 for what the script must refuse), then `share_hooks.py <root> --check` and the inventory clean of H001–H006. If H009 is present, report the inline Codex source separately because the script does not own it.
7. Run sync. Report the snapshot id, the events each host now carries, the Codex-only events/commands preserved, the Claude-only events left behind, and the host-specific trust requirements: Claude workspace/project trust and Codex project trust plus exact command-definition review (use `/hooks`; a command-string change requires review again).
