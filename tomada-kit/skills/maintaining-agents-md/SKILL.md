...
description: "Keep a project's agent rule files in one shape both Claude Code and Codex CLI obey: AGENTS.md is the master, each CLAUDE.md beside it is a thin stub importing it with `@AGENTS.md`. Modes: init (survey the project, draft AGENTS.md), audit (find and fix stale, bloated, missing, or host-specific rules), migrate (fold a legacy CLAUDE.md body, .claude/CLAUDE.md, and .claude/rules/*.md into AGENTS.md, then stub CLAUDE.md), sync (regenerate the stubs). Use when creating or improving AGENTS.md or CLAUDE.md, converting CLAUDE.md to AGENTS.md, cleaning up .claude/rules, folding a session's learnings into the project rules, or checking the rule files are in sync."
argument-hint: "[init | audit | migrate | sync] [project-root]"
metadata:
...
- Codex selects at most one non-empty instruction file per directory (`AGENTS.override.md`, then `AGENTS.md`, then configured fallback names), from the project root down to its **launch directory** only. `AGENTS.md` is the configured canonical name; on a case-sensitive filesystem, lowercase `agents.md` is not a default fallback (on macOS's usual case-insensitive filesystem it may resolve to the same path). The selected chain is concatenated under a configurable document budget (32 KiB by default); it has no import syntax and never opens CLAUDE.md. A subdirectory file reaches Codex only when Codex is started inside that directory. Source: https://learn.chatgpt.com/docs/agent-configuration/agents-md
...
**Input:** `$ARGUMENTS` = optional mode (`init` | `audit` | `migrate` | `sync`) and optional project root. No root → git toplevel of the working directory. No mode → run the inventory and follow its `suggested_mode`; say which mode was chosen and why before doing anything else. An explicit mode always wins over `suggested_mode`; when they disagree, say so in one line, proceed with the explicit mode, and report the findings the other mode would handle as fixes of the form "run `migrate`" rather than applying them.

**Reads:** every canonical and effective Codex instruction source (`AGENTS.md`, `AGENTS.override.md`, and configured fallback filenames), `CLAUDE.md`, `.claude/CLAUDE.md`, `CLAUDE.local.md`, `.claude/rules/**/*.md` under the root (skipping dependency and build directories), plus the relevant Codex config values; for `init`/`audit`, the project's manifests, task runners, CI config, and README.

**Writes:** `AGENTS.md` and `CLAUDE.md` files inside the project (root and subdirectories). Never `.claude/settings.local.json`. Before any modification — a snapshot at
...
Exit 1 from `inventory.py` means findings exist, not that the script failed. Read `findings`, `suggested_mode`, the effective Codex sources and budget, and the per-file `state` values (`stub`, `stub+extras`, `legacy`, `legacy-import`, `missing`, `orphan`, `malformed`). Finding codes R001–R013 are listed at the end of the script's `--help` and explained in [references/file-contract.md](references/file-contract.md).
...
5. Show the drafts, wait for the user's answer, snapshot (`python3 scripts/snapshot.py save <root> <files being overwritten> --label init --created <files being created>`), write the approved files, then run sync (step below).
...
2. Decide destinations with the table in the reference — global and pattern-only rules → a compact root AGENTS.md section whose heading carries the glob; directory-scoped rules → `<scope_dir>/AGENTS.md` plus its stub; Claude-only mechanics → the stub's free section. For every non-trivial `.claude/rules` block, run the semantic conversion pass in [references/semantic-rule-conversion.md](references/semantic-rule-conversion.md): mechanically copy only what is directly representable, and have the LLM rewrite the behavior, scope, exceptions, and examples when Claude's path-trigger semantics cannot be expressed exactly. A directory-scoped rule has one trade-off to surface: in its own directory it loads lazily in Claude Code but reaches Codex only when Codex is launched there; at the root (heading carries the glob) it is guaranteed on both hosts but costs root lines. Present both with the directory as the recommendation and wait for the answer.
...
5. Run sync. Report what moved where, what was deleted, and the snapshot id.
...
- [references/semantic-rule-conversion.md](references/semantic-rule-conversion.md) — the required LLM-assisted pass for Claude path-scoped rules and other non-mechanical migrations. Read in `migrate`.
...
- [references/platform-notes.md](references/platform-notes.md) — host-specific notes: how to wait for the user and how to delegate the survey.
- `scripts/inventory.py` — enumerate and classify; never writes. `scripts/sync_stubs.py` — the `sync` mode. `scripts/snapshot.py` — `save` / `list` / `restore`. Tests: `python3 -m unittest discover -s scripts/tests -p 'test_*.py'` from the skill directory.
...
- An active `AGENTS.override.md` or configured fallback is Codex's chosen source for that directory. Do not overwrite it automatically to make the two hosts look identical; report the divergence and preserve it unless the user explicitly resolves it.
